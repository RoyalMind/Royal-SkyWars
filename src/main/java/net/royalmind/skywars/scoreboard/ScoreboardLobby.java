...
public class ScoreboardLobby implements ScoreboardHandler {

    private RoyalSkyWars royalSkyWars;
    private final FileConfiguration config;
    private HighlightedString title;

    public ScoreboardLobby(final RoyalSkyWars royalSkyWars) {
    ...
    public String getTitle(Player player) {
        title = new HighlightedString(
                getConfig().getString("Scoreboard.Lobby.Title")
                , "&6&l", "&e&l"
        );
    ...
    public List<Entry> getEntries(Player player) {
        ...
        for (String s : getConfig().getStringList("Scoreboard.Lobby.Body")) {
